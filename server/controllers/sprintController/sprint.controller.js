...
async function searchSprintsController(req, res) {
  const sprintNamePattern = req.params.sprintName;
  try {
    const data = await sprintService.searchSprint(sprintNamePattern);
    res.status(200).json(data);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
