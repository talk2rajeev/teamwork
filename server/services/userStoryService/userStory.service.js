...
async function createUserStory(payload) {
  ...
  try {
    const [result, ...meta] = await pool.query(
      SQL_QUERIES.userStory.createUserStoryQuery,
      [
        title,
        description,
        statusId,
...
async function createBug(payload) {
  ...
  try {
    const [result, ...meta] = await pool.query(
      SQL_QUERIES.userStory.createBugQuery,
      [
        title,
        description,
        statusId,
...
async function getUserStoriesBySprintId(sprintId) {
  try {
    const [result, ...meta] = await pool.query(
      SQL_QUERIES.userStory.getUserStoriesBySprintId,
      [sprintId]
    );
    return result;
  } catch (err) {
...
async function getUserStoriesByProductId(productId) {
  try {
    const [result, ...meta] = await pool.query(
      SQL_QUERIES.userStory.getUserStoriesByProductId,
      [productId]
    );
    return result;
  } catch (err) {
...
async function getUserStoriesByEpicId(epicId) {
  try {
    const [result, ...meta] = await pool.query(
      SQL_QUERIES.userStory.getUserStoriesByEpicId,
      [epicId]
    );
    return result;
  } catch (err) {
...
async function getDetailedUserStoriesByUserStoryId(
  productId,
  sprintId,
  userStoryId
) {
  try {
    const [result, ...meta] = await pool.query(
      SQL_QUERIES.userStory.getDetailedUserStoriesByUserStoryId,
      [productId, sprintId, userStoryId]
    );
    return result;
  } catch (err) {
    throw new Error(
      `Failed to get detailed user stories by sprintId: ${err.message}`
    );
  }
}

async function getDetailedUserStoriesBySprintId(productId, sprintId) {
  try {
    const [result, ...meta] = await pool.query(
      SQL_QUERIES.userStory.getDetailedUserStoriesBySprintId,
      [productId, sprintId]
    );
    return result;
  } catch (err) {
...
async function getDetailedUserStoriesByProductId(productId) {
  try {
    const [result, ...meta] = await pool.query(
      SQL_QUERIES.userStory.getDetailedUserStoriesByProductId,
      [productId]
    );
    return result;
  } catch (err) {
    throw new Error(
      `Failed to get detailed user stories by productId: ${err.message}`
    );
  }
}
...
async function getDetailedUserStoriesByEpicId(epicId) {
  try {
    const [result, ...meta] = await pool.query(
      SQL_QUERIES.userStory.getDetailedUserStoriesByEpicId,
      [epicId]
    );
    return result;
  } catch (err) {
    throw new Error(
      `Failed to get detailed user stories by epicId: ${err.message}`
    );
  }
}
...
export {
  ...
  getDetailedUserStoriesByUserStoryId,
