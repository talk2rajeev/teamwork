...
async function createEpic(payload) {
  try {
    ...
    const result = await pool.query(SQL_QUERIES.epic.createEpicQuery, [
      epicName,
      epicDescription,
      createdById,
      productId,
...
async function getEpics(profileId) {
  try {
...
async function getEpicsByProductId(productId) {
  try {
    const result = await pool.query(SQL_QUERIES.epic.getEpicsByProductIdQuery, [
      productId,
    ]);
    return result[0];
  } catch (err) {
