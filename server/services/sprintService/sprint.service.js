...
async function createSprint(payload) {
    ...
    try{
        const [result, ...meta] = await pool.query(SQL_QUERIES.sprint.createSprintQuery, [sprintName, createdById, productId, startDate, endDate]);
        return result;
    } catch(err) {
...
async function getAllSprints() {
    try{
        const [result, ...meta] = await pool.query(SQL_QUERIES.sprint.getAllSprintQuery);
        return result;
    } catch(err) {
...
async function getSprintById(sprintId) {
    try{
        const [result, ...meta] = await pool.query(SQL_QUERIES.sprint.getSprintByIdQuery, [sprintId]);
        return result;
    } catch(err) {
...
async function getSprintByProductId(productId) {
    try{
        const [result, ...meta] = await pool.query(SQL_QUERIES.sprint.getSprintByProductIdQuery, [productId]);
        return result;
    } catch(err) {
...
export { createSprint, getAllSprints, updateSprint, getSprintById, getSprintByProductId };