...
type propsType = {
    label: string,
    type: buttonType,
    clickHandler: (event: React.MouseEvent<HTMLButtonElement, MouseEvent>, data?: any) => void,
    disabled?: boolean,
...
const Button = ({label, type, clickHandler, disabled, dataAttribute, data, loading}: propsType) => {
    ...
    return dataAttribute ? <button data-attr={dataAttribute} onClick={onclick} className={buttonClasses} disabled={disabled}>{buttonTextContent}</button>
        : <button onClick={onclick} className={buttonClasses} disabled={disabled}>{buttonTextContent}</button>;
}
